...
.electric-add-component
	el-form.device-add-form(:model="crop" label-width="100px"  center status-ico)
		...
		el-form-item(:label="$t('common.time')")
			el-date-picker(v-model="crop.time" type="daterange" range-separator="-" :start-placeholder="$t('common.startDate')" :end-placeholder="$t('common.endDate')" size="small")
		el-form-item(:label="$t('common.size')")
			el-row(:gutter="20" type="flex")
				...
				el-col(:span="16")
					el-select(v-model="crop.size.unit" :placeholder="$t('admin.crop.selectUnit')" size="small")
...
@Component({
	...
	methods: {
		...
		...mapActions('crop', ['create', 'getUnits']),
		loadData() {
