let currEncoding = localStorage.getItem('targetEncoding')

function targetEncodingIsTW() {
	return currEncoding === 'cn-tw'
}
...
function translateBody(obj) {
	...
	for (let i = 0; i < objs.length; i++) {
		if (
			...
			objs[i].tagName !== 'BR' &&
			objs[i].tagName !== 'IMG' &&
			objs[i].tagName !== 'CANVAS' &&
			objs[i].tagName !== 'I' &&
			objs[i].tagName !== 'SCRIPT' &&
			objs[i].tagName !== 'TIME'
		) {
...
export default function translatePage() {
	let targetEncoding = 'cn-tw'
	if (targetEncodingIsTW()) {
		targetEncoding = 'cn'
	}
	currEncoding = targetEncoding
